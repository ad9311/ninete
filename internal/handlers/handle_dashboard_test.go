	...
	cases := []struct {
	...
	}{
		{
			...
			fn: func(t *testing.T) {
				...
				cookies := s.AuthCookies(t, "macros_user_1@example.com", "macros_password_1")

				req := spec.NewGetRequest("/macros?meal_type=breakfast", cookies)
				rec := httptest.NewRecorder()
				handler.ServeHTTP(rec, req)

				require.Equal(t, http.StatusOK, rec.Code)
				body := rec.Body.String()
				require.Contains(t, body, "300")
				require.NotContains(t, body, "750")
			},
		},
	}
