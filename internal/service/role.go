...
// CreateNewRole creates a new role with the given name, ensuring uniqueness. Returns an error if the role already exists or creation fails.
...
// FindRoleByName retrieves a role from the database by its name. Returns an error if the role is not found or lookup fails.
