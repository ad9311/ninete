...
| API (SPA) | `/api/session`, `/api/categories`, `/api/recurrent-expenses`, and the resource routes later phases add | `api.go`, `handle_api_session.go`, `handle_api_categories.go`, `handle_api_recurrent_expenses.go` | reuses the page handlers' stores |
