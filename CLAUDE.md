...
| API (SPA) | `/api/session`, `/api/categories`, `/api/dashboard`, `/api/exports/expenses.json`, `/api/delete-data` (+ `/expenses`, `/recurrent-expenses`, `/expense-budgets`, `/tags`), `/api/recurrent-expenses`, `/api/expenses` (+ `/quick`, `/stats`, `/budgets`), and the resource routes later phases add | `api.go`, `handle_api_session.go`, `handle_api_categories.go`, `handle_api_dashboard.go`, `handle_api_exports.go`, `handle_api_delete_data.go`, `handle_api_recurrent_expenses.go`, `handle_api_expenses.go`, `handle_api_quick_expense.go`, `handle_api_expense_stats.go`, `handle_api_expense_budgets.go` | reuses the page handlers' stores |
